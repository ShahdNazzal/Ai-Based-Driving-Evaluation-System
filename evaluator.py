...
from config import (
    ...
    OVERLAP_PERFECT_THRESH,
    OVERLAP_GOOD_THRESH,
    OVERLAP_TOUCH_THRESH,
    COLLISION_CM,
    TOO_CLOSE_CM,
    TOO_FAR_CM,
    VOTE_WINDOW,
...
class Evaluator:
    ...
    def evaluate_frame(self, car_box, zone, cone_centers_px=None):
        ...
        # ── 1. CENTROID — smooth curve from first touch ───────────────
        if frac <= 0.0:
            sb.centroid = 0.0
        elif frac < OVERLAP_TOUCH_THRESH:
            t = frac / OVERLAP_TOUCH_THRESH
            sb.centroid = CENTROID_WEIGHT * 0.10 * t
        elif frac < OVERLAP_GOOD_THRESH:
            t = ((frac - OVERLAP_TOUCH_THRESH) /
                 (OVERLAP_GOOD_THRESH - OVERLAP_TOUCH_THRESH))
            sb.centroid = CENTROID_WEIGHT * (0.10 + 0.60 * t)
        elif frac < OVERLAP_PERFECT_THRESH:
            t = ((frac - OVERLAP_GOOD_THRESH) /
                 (OVERLAP_PERFECT_THRESH - OVERLAP_GOOD_THRESH))
            sb.centroid = CENTROID_WEIGHT * (0.70 + 0.30 * t)
        else:
            sb.centroid = float(CENTROID_WEIGHT)
        ...
        if diff_deg <= HEADING_TOLERANCE_DEG:
            sb.heading = float(HEADING_WEIGHT)
        else:
        ...
        ok_count   = sum(1 for m in margins if m >= -30)
        sb.corners = CORNER_WEIGHT * ((ok_count / 4.0) ** 0.5)

        # ── 4. BALANCE + COLLISION ────────────────────────────────────
        centroid_cm  = corners_cm.mean(axis=0)
        active_cones = (cone_centers_px
                        if (cone_centers_px and len(cone_centers_px) >= 2)
                        else zone.cone_centers_px)
        ...
        if active_cones and len(active_cones) >= 2:
            ...
            if d1 < COLLISION_CM:
                ...
                sb.collision = float(COLLISION_WEIGHT)
            ...
            if avg_d < TOO_CLOSE_CM:
                ...
                sb.balance = float(BALANCE_WEIGHT)
            elif balance < 50:
                sb.balance = BALANCE_WEIGHT * 0.8
            else:
                sb.balance = max(
                    0.0, BALANCE_WEIGHT * (1.0 - (balance - 50.0) / 100.0))
        else:
            sb.balance   = float(BALANCE_WEIGHT)
            sb.collision = float(COLLISION_WEIGHT)
...
def _estimate_heading(box):
    x1, y1, x2, y2 = box
    return 0.0 if (x2 - x1) >= (y2 - y1) else math.pi / 2.0
